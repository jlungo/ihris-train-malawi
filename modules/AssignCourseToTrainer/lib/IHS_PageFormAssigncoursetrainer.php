	...
	* @access public
	*/
	class IHS_PageFormAssigncoursetrainer extends I2CE_PageForm {
	...
	protected function applyLimits($assign_course_trainer){
	   ...
	   $user_info=iHRIS_PageFormLecturer::fetch_user_info($username);
		...
		else if($this->getUser()->role=="registrar") {
			$where_courses=array(	"operator"=>"AND",
											"operand"=>array(0=>array(	"operator"=>"FIELD_LIMIT",
																				"field"=>"training_institution",
																				"style"=>"equals",
																				"data"=>array("value"=>$inst_id)
	...
	protected function save() {
		...
		foreach($lecturers as $lecturer) {
			foreach($training_courses as $training_course) {
				...
				$course_semester=$trainObj->getField("semester")->getDBValue();
				list($form,$sem)=array_pad(explode("|",$course_semester,2),2,"");
				$month=date("m");
				if($month>=7 and $month<=12 and $sem % 2==0)
				$sem=$sem-1;
				else if($month>=1 and $month<=6 and $sem % 2!=0)
				$sem=$sem+1;
				$course_semester="semester|".$sem;
				$assign_course_trainer->getField("semester")->setFromDB($course_semester);
				...
				if($course_type!="course_type|general_education") {
