	...
	* @access public
	*/
	class IHS_PageFormRejoiningStudentProcess extends I2CE_PageForm {
		/**
	...
	if(!$this->hasPermission("task(person_can_edit)" or $this->getUser()->role=="admin")) {
		...
		if($this->isPost()) {
			...
			$persObj->populateChildren("discontinued");
			foreach($persObj->getChildren("discontinued") as $discoObj) {
				if($discoObj->getField("registration")->getDBValue() == $this->student_registration["id"]) {
					$this->template->setForm($discoObj);
					break;
				}
				}
			...
			if($discoObj->getField("recommendations")->getDisplayValue()=="recommendations|FD")
			$rejoin_reason="rejoin_reaso|discontinued";
			else if($discoObj->getField("recommendations")->getDisplayValue()=="recommendations|FE")
			$rejoin_reason="rejoin_reason|fail_and_exclude";
			$rejoinObj->getField("rejoin_reason")->setFromDB($rejoin_reason);
			}
			
		if($this->request_exists("id")) {
			$this->student_registration=STS_PageFormPerson::load_current_registration($this->request("id"));
			$persObj=$this->factory->createContainer($this->request("id"));
			$persObj->populate();
			...
			$rejoinObj=$this->factory->createContainer("rejoin|0");
			$rejoinObj->populate();
			$rejoinObj->setParent($this->request("id"));
			$this->applyLimits($rejoinObj);
			}
