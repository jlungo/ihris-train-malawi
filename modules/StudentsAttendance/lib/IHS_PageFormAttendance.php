 ...
 * @access public
 */
class IHS_PageFormAttendance extends I2CE_PageForm {
    ...
    protected function loadObjects() {
        if ($this->isPost()) {
            ...
            if($this->post["form"]["attendance"][0][0]["fields"]["out_of_inst"]=="yesno|yes") {
            	$parent=$this->post["form"]["attendance"][0][0]["fields"]["parent"];
            	...
            	if(count($persObj->getChildren("discontinued"))==0) {
            		$discoObj=$this->factory->createContainer("discontinued");
            		$persObj->populate();
            		$discoObj->getField("academic_year")->setFromDB($student_registration["academic_year"]);
            		$discoObj->getField("registration_number")->setValue($student_registration["registration_number"]);
            		$discoObj->getField("disco_reason")->setFromDB("disco_reason|misconduct");
            		$discoObj->getField("recommendations")->setFromDB("recommendations|FD");
            		$student_registration=STS_PageFormPerson::load_current_registration($parent);
            		$discoObj->getField("registration")->setFromDB($student_registration["id"]);
            		$discoObj->getField("parent")->setValue($parent);
            		$user=new I2CE_User;
            		$discoObj->save($user);
            		}
            	}
